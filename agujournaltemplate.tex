...
\begin{table}[p]
\caption{Key HYPE model parameters and their calibration ranges in the aggregate PEST run. $N_{\mathrm{adj}}/N_{\mathrm{cls}}$ gives the number of class variants PEST adjusted out of the total number of land use or soil class variants defined for that parameter across the 27-catchment domain; parameters with $N_{\mathrm{adj}}=0$ were held at a fixed literature or default value rather than calibrated. Prior range is the lower/upper PEST bound (a single value where all variants share the same fixed value); calibrated value is the value(s) after the aggregate calibration run, given as a range where variants converged to different values. Land use and soil class dependency follows the SLC scheme described in Section~2.1; parameter descriptions follow standard HYPE documentation \cite{Lindstrom2010} and should be verified against the project's own model configuration before final submission.}
